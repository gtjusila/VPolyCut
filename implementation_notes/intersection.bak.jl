...
function get_corner_polyhedron(sepa::IntersectionSeparator)
    ...
    for (k, index) in enumerate(basis_indices)
        if index >= 0
            ...
            SCIP.@SCIP_CALL SCIP.LibSCIP.SCIPgetLPBInvRow(
                sepa.scipd, k - 1, pointer(tableau[index + 1], col_num + 1), C_NULL, C_NULL
            )
            SCIP.@SCIP_CALL SCIP.LibSCIP.SCIPgetLPBInvARow(
                sepa.scipd,
                k - 1,
                pointer(tableau[index + 1], col_num + 1),
                pointer(tableau[index + 1]),
                C_NULL,
                C_NULL
            )
        end
...
function computeIntersectionPoints(
    scipd::SCIP.SCIPData, current_solution, split_index, ray_collection
)
    intersection_points = []
    pararrel_ray = []
...
function SCIP.exec_lp(sepa::IntersectionSeparator)
    ...
    intersection_points = computeIntersectionPoints(
        sepa.scipd, lp_sol, split_var, rays_collection
    )
    lpi = Ref{Ptr{SCIP.SCIP_LPI}}(C_NULL)
    SCIP.@SCIP_CALL SCIP.SCIPlpiCreate(
        lpi, C_NULL, "Seperating LP", SCIP.SCIP_OBJSEN_MINIMIZE
    )
    # Add Variables
    SCIP.@SCIP_CALL SCIP.SCIPlpiAddCols(
        ...
        dim,
        ones(dim),
        -SCIP.SCIPinfinity(sepa.scipd) * ones(dim),
        SCIP.SCIPinfinity(sepa.scipd) * ones(dim),
        C_NULL,
        0,
        C_NULL,
    ...
    SCIP.@SCIP_CALL SCIP.SCIPcreateRowSepa(
        sepa.scipd,
        row,
        sepa.scipd.sepas[sepa],
        "",
        nnonz,
        pointer(cols__),
        pointer(vals__),
        b,
        SCIP.SCIPinfinity(sepa.scipd),
        true,
        false,
        false
    )
