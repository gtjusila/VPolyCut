...
It is also important to note that SCIP does not give you the matrix $A'$ but $A$. The matrix $A' = [A E]$ where $E$ is an mxm matrix with entry {+1,-1} (corresponding to the slack variable). So the tableau of A' is $[B^{-1}A B^{-1}']$ where $B^{-1}'$ denote the matrix $B^{-1}
