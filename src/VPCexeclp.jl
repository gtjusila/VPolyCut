...
function _exec_lp(sepa::VPCSeparator)
    ...
    try
        ...
        elseif e isa FailedCBarTest
            @debug "Failed CBar Test"
            error_occurred = true
            sepa.termination_message = "FAILED_CBAR_TEST"
        elseif e isa PStarNotTight
            @debug "P* Not Tight"
            error_occurred = true
            sepa.termination_message = "PSTAR_NOT_TIGHT"
        elseif e isa PStarUnbounded
            @debug "P* Unbounded"
            error_occurred = true
            sepa.termination_message = "PSTAR_UNBOUNDED"
        else
            rethrow(e)
        end
    end
