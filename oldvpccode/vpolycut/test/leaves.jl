...
@testset "Leaves test 1" begin
    ...
    firstlp = initiate_callback(model, data) do data
        scip = data["scip"]
        branchandbound = VPolyCut.BranchAndBound(scip; max_leaves=2)

        # ILP is feasible
        VPolyCut.execute_branchandbound(branchandbound)
        for leaf in VPolyCut.get_leaves(branchandbound)
            @info leaf
        end
...
@testset "Leaves test 3" begin
    ...
    firstlp = initiate_callback(model, data) do data
        scip = data["scip"]
        branchandbound = VPolyCut.BranchAndBound(scip)

        # ILP is feasible
        @test VPolyCut.execute_branchandbound(branchandbound) == true

        x = VPolyCut.get_best_solution(branchandbound)
        # Check Solution
        @test x[1] == 1 && x[2] == 6

        # Obj Sense is Max so reverse
        obj = VPolyCut.get_primal_bound(branchandbound)
        @test obj == -20
